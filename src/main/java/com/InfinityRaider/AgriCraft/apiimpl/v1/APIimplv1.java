...
public class APIimplv1 implements APIv1 {
	...
	public ISeedRequirements getSeedRequirements(ItemStack seed) {
		if (SeedHelper.isValidSeedStack(seed)) {
			...
			result.setHasRequiredDimension(false);
			result.setRequiredDimenstions(null);
			result.setHasRejectedDimension(false);
			result.setRejectedDimenstions(null);
		...
		if (SeedHelper.isValidSeedStack(seed)) {
			...
			if (te instanceof TileEntityCrop) {
				...
				if(!growthRequirement.isValidSoil(world, x, y-1, z)) {
					return SeedRequirementStatus.WRONG_BASEBLOCK;
				}
